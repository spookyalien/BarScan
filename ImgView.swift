                ...
                for index in 0..<observations.count {
                    if let topCandidate = observations[index].topCandidates(1).first {
                        ...
                        // Accounts for issue of backroom letter O being mistaken as 0
                            ...
                            let back_loc = processed_input.index(processed_input.startIndex, offsetBy: 6)
                        ...
                        else if processed_input.range(of: "^\\d{2}[A-Za-z]\\d{2}[A-Za-z]\\d{3}", options: .regularExpression, range: nil, locale: nil) != nil {
